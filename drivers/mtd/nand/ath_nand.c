...
ath_check_all_0xff(ath_nand_sc_t *sc, unsigned addr0, unsigned addr1, unsigned *all_0xff)
{
	...
	i = ath_nand_status(sc, NULL) & ATH_NF_RD_STATUS_MASK;
 	...
 	memcpy(buf, (void*)KSEG1ADDR(pa), count);	// cache sync equivalent
...
ath_nand_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	for (j = 0, i = s_first; j < n; j++, i += mtd->erasesize) {
		...
		if ((ret = ath_nand_block_erase(sc, ba0, ba1)) != 0) {
			printf("%s: erase failed 0x%llx 0x%x 0x%x %lx "
				"%lx %lx\n", __func__,
				(unsigned long long) instr->addr, n,
				mtd->erasesize, i, (unsigned long)ba1,
				(unsigned long)ba0);
			break;
		}
		...
		struct mtd_oob_ops *ops)
{
	...
	pa = (void *)virt_to_phys(ath_nand_io_buf);
	if (!rd) flush_cache((unsigned)ath_nand_io_buf, mtd->writesize + mtd->oobsize);	// for writes...
...
ath_nand_block_isbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	for (i = 0; i < 2; i++, ofs += mtd->writesize) {
		if (ath_nand_read_oob(mtd, ofs, &ops) ||
			ops.oobretlen != ops.ooblen) {
			printk("%s: oob read failed at 0x%x\n", __func__, (unsigned)ofs);
...
nand_param_page(ath_nand_sc_t *sc, uint8_t *buf, unsigned count)
{
	...
	memcpy(buf, (void*)KSEG1ADDR(buf), count);	// get into the cache
...
ath_nand_hw_init(ath_nand_sc_t *sc, void *p)
{
	...
	if (p) {
		...
		pa = p;
		debug("Ath Nand ID[%p]: %02x:%02x:%02x:%02x:%02x\n",
...
void ath_nand_init(void)
{
	debug(DRV_DESC ", Version " DRV_VERSION
...
// 		printf("NAND error nand-scan-ident\n");

//  	if (nand_scan_tail(mtd))
//  		printf("NAND error nand_scan_tail\n");

	if (nand_register(nand_curr_device))
 		printf("NAND error nand_register\n");
		//error out
	return;
}
