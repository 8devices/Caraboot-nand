...
#define sysMsDelay(_x) mdelay(_x)
 ...
 */
	...
	athrs17_reg_write(S17_P6STATUS_REG, 0x7e);	
        athrs17_vlan_config();
#endif
       //Set 1.8V RGMII interface
       athrs17_reg_write(S17_MAC_PWR_SEL, (athrs17_reg_read(S17_MAC_PWR_SEL)| S17_PWR_RGMII_0 | S17_PWR_RGMII_1));
...
static void extract_range(char * input, unsigned int * plo, unsigned int * phi)
{
	char * end;
...
static int do_athreg(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	char		op[2];
	unsigned int	addrlo, addrhi, reglo, reghi;
	 ...
	 */
	if (op[0] == 'r') {
		for (addr = addrlo; addr <= addrhi; addr+=4) {
			data = athrs17_reg_read(addr);
			printf("addr=%x data=%x\n", addr, data);
		}
	} else if (op[0] == 'w') {
		addr = addrlo;
		reg = reglo;
		athrs17_reg_write(addr, reg);
 		printf("writing to the PHY addr=%x val=%x\n",	addr, reg);
	} else if (op[0] == 'z') {
		u32 i, j, val;
		for (i=0;i<=0xC80;i+=(4*8)){
			printf("%04x: ", i);
			for (j=0;j<32;j+=4){
				val = athrs17_reg_read( i+j);
				printf(" 0x%08x", val);
			}
			printf("\n");
		}
...
U_BOOT_CMD(
	ar,	5,	1,	do_athreg,
	"manipulate Atheros QCA833x switch registers",
	"ar r <addr>       - read\n"
	"ar w <addr> <val> - write\n"
	"ar z              - dump range (0x0 - 0xC80)\n"
	"<addr> - can be range ex.: 0x0-0x10"
);
#endif