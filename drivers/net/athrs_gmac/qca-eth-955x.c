...
static int
ath_gmac_send(struct eth_device *dev, void *packet, int length)
{
	int i;
...
static void ath_gmac_hw_start(ath_gmac_mac_t *mac)
{
	 ...
	 */
	//Broadcast, multicast recieve enabled for uIP
...
static int ath_gmac_setup_fifos(ath_gmac_mac_t *mac)
{
	...
	if (ath_gmac_alloc_fifo(PKTBUFSRX, mac->fifo_rx))
		return 1;
...
ath_gmac_mac_addr_loc(void)
{
	...
	ret = nand_art_get_cal_data(&eeprom);
	if(!ret) {
		return eeprom;
	}
	else {
		eeprom = malloc(12);
		memset(eeprom, 0xff, 12);
		return eeprom;
	}
}
