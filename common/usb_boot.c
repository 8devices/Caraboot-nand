/*
 * USB boot functions for Rambutan
 *
...
int usbboot_boot(char * boot_dev_part, char * boot_file_name)
{
	...
	loff_t size = 0;
	char addr_str[20];
	long long max_size = (gd->ram_size/2) - (4*1024*1024);
	long load_addr = 0x80000000 + (gd->ram_size/2);
	sprintf(addr_str, "%x", load_addr);

	char* bootm_argv[] = {"bootm", addr_str};
	...
	fat_read_ret = do_fat_read_at(boot_file_name,
					...
					&size);
	if (fat_read_ret == 0){
		printf ("Booting image %s (%llu bytes) from usb device %s \n",
...
int usbboot_recovery(char * boot_dev_part, char * boot_file_name)
{
	...
	fat_read_ret = do_fat_read_at(boot_file_name,
					...
					(long long)CFG_MAX_USB_RECOVERY_FILE_SIZE,
					0,
					0,
					&size);
	...
	if (fat_read_ret == 0){
		...
		char* load_addr = MK_STR(CFG_USB_RECOVERY_LOAD_ADDR);
		sprintf(flash_cmd,"nand erase.part %s; nand write %s %s %s",
...
int usbboot_scan(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	// scan all devicesi and partitions for boot image
	for (devno = 0; ; ++devno) {
